...
import { resourceDefinitions, resourceOrder, getResourceDefinition } from "./resource-definitions";
...
describe("resourceDefinitions", () => {
  ...
  it("can derive localized resource copy from the translator entrypoint", () => {
    const chineseDefinition = getResourceDefinition("applications", (key, _params, fallback) => {
      if (key === "resource.oauthClients.title") {
        return "应用管理";
      }
      if (key === "resource.oauthClients.description") {
        return "管理应用、回调地址、scope 和 App Secret。";
      }
      return fallback ?? key;
    });

    expect(chineseDefinition.title).toBe("应用管理");
    expect(chineseDefinition.description).toBe("管理应用、回调地址、scope 和 App Secret。");
  });
});
