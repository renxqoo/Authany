...
import { AdminV2ApiError, adminV2Fetch } from "./resource-client";
import type { ResourceAction, ResourceKey } from "./types";

export function AdminV2ResourceDetailPage({
  resourceKey,
  id
}: {
  ...
  const load = useCallback(async () => {
    setError("");
    try {
      setRecord(await adminV2Fetch<Record<string, unknown>>(`${definition.endpoint}/${id}`));
    } catch (loadError) {
  ...
  }, [definition.endpoint, id, pathname, router]);
  ...
  async function save(payload: Record<string, unknown>) {
    await adminV2Fetch(`${definition.endpoint}/${id}`, {
      method: "PATCH",
  ...
  async function runAction(action: ResourceAction, confirmValue = "") {
    ...
    setError("");
    try {
      await adminV2Fetch(action.endpoint(id, record), {
    ...
    <div className="space-y-6">
      <section className="rounded-[36px] border border-slate-200 bg-[linear-gradient(135deg,rgba(240,249,255,0.98),rgba(255,255,255,0.98)_45%,rgba(255,251,235,0.9))] p-8 shadow-sm">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <Link className="text-sm font-medium text-sky-700 hover:text-sky-900" href={definition.path}>← Back to {definition.title}</Link>
            ...
            {headerStatus ? <div className="mt-4">{renderResourceValue(headerStatus, "status")}</div> : null}
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            {definition.editFields ? (
              <Button onClick={() => setEditing(true)} type="button" variant="secondary">Edit</Button>
            ) : null}
          </div>
        </div>
      </section>
            ...
            <CardContent className="grid gap-5 md:grid-cols-2">
              {section.fields.map((field) => (
                <div className="space-y-2" key={field.label}>
                  ...
                  <div className="text-sm text-slate-700">{renderResourceValue(readValue(record, field), field.kind)}</div>
                </div>
              ))}
            </CardContent>
          </Card>
        ...
        <Card className="rounded-[28px] border-red-200 bg-red-50/50">
          ...
          <CardContent className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="max-w-2xl text-sm leading-6 text-red-700">
              {definition.dangerAction.confirmDescription ?? "This action is destructive and should be used carefully."}
            </div>
      ...
      {editing && definition.editFields ? (
        <ResourceFormDialog
          ...
          onSubmit={save}
          title={`Edit ${definition.titleValue(record)}`}
        />
      ) : null}
      ...
      {activeAction ? (
        <ResourceConfirmDialog
          ...
          description={activeAction.confirmDescription ?? "Confirm this action to continue."}
