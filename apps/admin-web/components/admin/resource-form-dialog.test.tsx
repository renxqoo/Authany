...
import * as resourceClient from "@/lib/admin/resource-client";
...
describe("ResourceFormDialog", () => {
  ...
  it("keeps target connection textarea and number inputs editable", async () => {
    renderWithI18n(
      <ResourceFormDialog
        fields={[
          {
          ...
          },
          {
            ...
            type: "select",
            dependsOn: { field: "principal_type", values: ["agent"] },
            options: [{ label: "Demo Agent / agt_demo", value: "agt_demo" }]
          },
          {
            name: "target_resource",
            label: "Target resource",
            type: "select",
            options: [{ label: "Demo Target / demo-target", value: "demo-target" }]
          },
          {
    ...
    fireEvent.change(screen.getByLabelText("Agent ID"), { target: { value: "agt_demo" } });
    fireEvent.change(screen.getByLabelText("Target resource"), { target: { value: "demo-target" } });
    fireEvent.change(screen.getByLabelText("External context mode"), { target: { value: "optional" } });
    fireEvent.change(screen.getByLabelText("Allowed context providers"), { target: { value: "provider.alpha\nprovider.beta" } });
    fireEvent.change(screen.getByLabelText("Max token TTL seconds"), { target: { value: "600" } });

    expect(screen.getByLabelText("Allowed context providers")).toHaveValue("provider.alpha\nprovider.beta");
    expect(screen.getByLabelText("Max token TTL seconds")).toHaveValue(600);
  });

  it("does not reset in-progress values when parent rerenders with equivalent fields", () => {
    const props = {
      fields: [
        {
          name: "principal_type",
    ...
    const { rerender } = renderWithI18n(<ResourceFormDialog {...props} />);
    ...
    rerender(
      <I18nProvider initialLocale="en">
        <ResourceFormDialog
          {...props}
          fields={props.fields.map((field) => ({ ...field }))}
        />
      </I18nProvider>,
    );

    expect(screen.getByLabelText("Principal type")).toHaveValue("agent");
    expect(screen.getByLabelText("Allowed context providers")).toHaveValue("provider.alpha");
    expect(screen.getByLabelText("Max token TTL seconds")).toHaveValue(600);
  });
});
