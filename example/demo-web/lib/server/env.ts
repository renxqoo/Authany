export function getDemoEnv() {
  ...
  const runtimeId = process.env.DEMO_RUNTIME_ID?.trim();
  ...
  if (!authanyBaseUrl || !clientId || !clientSecret || !redirectUri || !sessionSecret || !agentId || !agentCredential || !targetResource || !targetServiceUrl) {
    throw new Error("AUTHANY_INTERNAL_URL, DEMO_WEB_CLIENT_ID, DEMO_WEB_CLIENT_SECRET, DEMO_WEB_REDIRECT_URI, DEMO_WEB_SESSION_SECRET, DEMO_AGENT_ID, DEMO_AGENT_CREDENTIAL, DEMO_TARGET_RESOURCE, and DEMO_TARGET_SERVICE_URL are required.");
  }
  return {
    authanyBaseUrl,
    clientId,
