...
主服务配置参考根目录 [`.env.example`](/Users/wrr/work/authany/.env.example:1)。
...
```http
POST /api/admin/agents/{agentDbId}/runtimes
Content-Type: application/json

{
  "runtime_id": "runtime_search_cli",
  "runtime_type": "cli",
  "runtime_mode": "stateless",
  "credential_delivery_mode": "bearer",
...
```http
POST /api/admin/agents/{agentDbId}/credentials
Content-Type: application/json

{
  "credential_type": "bearer_secret"
}
```
...
```json
{
  "data": {
    ...
    "credential_type": "bearer_secret",
    "hint": "sk_live_...xxxx",
    "secret": "sk_live_abc123def456"    // 仅此一次可见
  }
}
```
...
```http
POST /api/admin/target-resources
Content-Type: application/json

{
  ...
  "token_validation_mode": "local_jwt",
  "trust_config": {}
}
```
...
```http
# 创建连接: Agent → 目标资源
POST /api/admin/target-resources/{targetDbId}/connections
Content-Type: application/json

{
  "principal_type": "agent",
  "principal_id": "agent_prod_search",
  "runtime_registration_id": "<runtimeDbId>",
...
# 创建授权: 允许该连接访问
POST /api/admin/connections/{connectionDbId}/grants
Content-Type: application/json

{
  "grant_type": "platform_authorized",
  "effect": "allow",
  "constraints": {},
...
| 操作       | 方法   | 路径                                    |
| ---------- | ------ | --------------------------------------- |
| 列表       | GET    | `/api/admin/applications`               |
| 创建       | POST   | `/api/admin/applications`               |
| 详情       | GET    | `/api/admin/applications/:id`           |
| 更新       | PATCH  | `/api/admin/applications/:id`           |
| 轮换密钥   | POST   | `/api/admin/applications/:id/secrets/rotate` |

### Agent 管理

| 操作         | 方法   | 路径                                          |
| ------------ | ------ | --------------------------------------------- |
| 列表         | GET    | `/api/admin/agents`                           |
| 创建         | POST   | `/api/admin/agents`                           |
| 详情         | GET    | `/api/admin/agents/:id`                       |
| Runtime 列表 | GET    | `/api/admin/agents/:id/runtimes`              |
| Runtime 创建 | POST   | `/api/admin/agents/:id/runtimes`              |
| 凭据列表     | GET    | `/api/admin/agents/:id/credentials`           |
| 签发凭据     | POST   | `/api/admin/agents/:id/credentials`           |
| 吊销凭据     | POST   | `/api/admin/agents/:id/credentials/:cid/revoke` |

### 目标资源管理

| 操作     | 方法   | 路径                                                    |
| -------- | ------ | ------------------------------------------------------- |
| 列表     | GET    | `/api/admin/target-resources`                           |
| 创建     | POST   | `/api/admin/target-resources`                           |
| 详情     | GET    | `/api/admin/target-resources/:id`                       |
| 连接列表 | GET    | `/api/admin/target-resources/:id/connections`           |
| 创建连接 | POST   | `/api/admin/target-resources/:id/connections`           |
| 授权列表 | GET    | `/api/admin/connections/:id/grants`                     |
| 创建授权 | POST   | `/api/admin/connections/:id/grants`                     |
| 吊销授权 | POST   | `/api/admin/connections/:id/grants/:gid/revoke`         |

### 密钥管理

| 操作     | 方法   | 路径                         |
| -------- | ------ | ---------------------------- |
| 列表     | GET    | `/api/admin/keys`            |
| 生成     | POST   | `/api/admin/keys`            |
| 激活     | POST   | `/api/admin/keys/:id/activate` |
| 退役     | POST   | `/api/admin/keys/:id/retire` |

### 审计日志

| 操作 | 方法 | 路径                    |
| ---- | ---- | ----------------------- |
| 查询 | GET  | `/api/admin/audit`      |
