...
> 分析范围: `/server/src` 目录全部源码 + `server/prisma/schema.prisma`
...
### 3.2 模块注册: `server/src/app.module.ts`
...
| 实体               | 路径前缀                              | 说明               |
| ------------------ | ------------------------------------- | ------------------ |
| Applications       | `/api/v1/admin/applications`           | OAuth 客户端 + 密钥管理 |
| Agents             | `/api/v1/admin/agents`                 | Agent 身份管理     |
| Runtimes           | `/api/v1/admin/runtimes`               | 运行时注册         |
| Credentials        | `/api/v1/admin/agents/:id/credentials` | 调用凭据           |
| Target Resources   | `/api/v1/admin/target-resources`       | 目标资源注册       |
| Connections        | `/api/v1/admin/target-connections`     | 目标连接           |
| Grants             | `/api/v1/admin/access-grants`          | 访问授权           |
| Keys               | `/api/v1/admin/keys`                   | RSA 密钥对轮换     |
| Audit              | `/api/v1/admin/audit-events`           | 审计日志查询       |
| Metrics            | `/api/v1/admin/metrics`                | 指标快照           |
